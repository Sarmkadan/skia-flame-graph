...
public class ChromeTraceParserTests : IChromeTraceParserTests
{
    ...
    {
        var events = ChromeTraceParser.Deserialize(ChromeTraceParserTestsConstants.SimpleCompleteEventsJson);
        Assert.Equal(3, events.Length);
        Assert.Equal(ChromeTraceParserTestsConstants.MainEventName, events[0].Name);
        Assert.Equal(ChromeTraceParserTestsConstants.EventPhaseComplete, events[0].Ph);
        Assert.Equal(0, events[0].Ts);
        Assert.Equal(100, events[0].Dur);
        Assert.Equal(ChromeTraceParserTestsConstants.ThreadId1, events[0].Tid);
    }
    ...
    {
        var root = ChromeTraceParser.BuildTree(ChromeTraceParser.Deserialize(ChromeTraceParserTestsConstants.NestedEventsJson));

        // Should have one thread node
        Assert.Single(root.Children);
        var threadNode = root.Children[0];
        Assert.StartsWith(ChromeTraceParserTestsConstants.ThreadNamePrefix, threadNode.Name);

        // Thread should have main as child
        Assert.Single(threadNode.Children);
        var main = threadNode.Children[0];
        Assert.Equal(ChromeTraceParserTestsConstants.MainEventName, main.Name);
    ...
    {
        var root = ChromeTraceParser.BuildTree(ChromeTraceParser.Deserialize(ChromeTraceParserTestsConstants.MultipleThreadsJson));
