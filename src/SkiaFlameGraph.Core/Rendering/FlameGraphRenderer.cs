...
{
    private static readonly SKColor RootBarColor = new(0x3a, 0x3a, 0x44);
    private const string ElidedFrameLabel = "[...]";
    private const float LabelHorizontalPadding = 6f;
    private const float LabelLeftInset = 3f;
    private const float LabelBaselineFactor = 0.35f;
        ...
        SKFont font, SKPaint stroke, SKPaint textPaint)
    {
        ...
        if (node.Children.Count > 0 && !_options.ShouldRenderSubtree(width))
        {
            ...
            if (_options.ShouldRenderFrame(width))
            {
                ...
                var elidedFill = GetPaintForColor(elidedFillColor);
    ...
    private void DrawLabel(SKCanvas canvas, string text, SKRect rect, SKFont font, SKPaint paint)
    {
        ...
        var baseline = rect.MidY + _options.FontSize * LabelBaselineFactor;
        canvas.Save();
        canvas.ClipRect(rect);
        canvas.DrawText(display, rect.Left + LabelLeftInset, baseline, SKTextAlign.Left, font, paint);
        canvas.Restore();
    }
